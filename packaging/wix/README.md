...
SpacePilot's MSI flow is generated by `scripts\build-msi.ps1` from the published app folder. The script keeps the WiX file list in generated build output so the installer always matches the current publish contents.
